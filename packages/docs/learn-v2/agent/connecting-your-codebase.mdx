...
Each Subframe project connects to one local folder at a time. Use a separate Subframe project for each folder so your designs and code stay paired.
...
The agent can now read and edit files inside that folder and start its development server.
