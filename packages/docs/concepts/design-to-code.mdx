...
By default, Subframe **determinstically** generates pure UI + CSS code without application logic (handling state, API calls, etc). Developers are expected to add this logic themselves after export.
...
In a nutshell, **components are synced, pages are exported**. This is because pages are typically modified with business logic like API calls after export. If your component needs logic after syncing, see our guide on best practices for [exporting components](/developing/exporting-components).

## Design handoff

Say your designer creates a sign in page in Subframe:

<Frame>
  <img src="/images/developers/getting-started-signin-page.png" alt="Sign in page designed in Subframe" />
</Frame>
...
<Frame>
  <img src="/images/concepts/design-to-code-annotated.png" alt="Sign in page with components highlighted" />
</Frame>

### Syncing components

When the designs are ready for handoff, the developer syncs these components to their codebase by running the following command:
...
The developer can also sync all design system components at once:
...
For more information on syncing components, see the [Syncing components](/developing/syncing-components) guide.

### Exporting pages

Once the components are synced, the generated page code can now be exported. The generated code has stubs for business logic that need to be filled in by the developer:

<SignInPageCode />

The page code can now be exported in two ways:

- **MCP server (recommended)** - install Subframe MCP server and ask your AI tool to integrate the page code directly using the [page link](/developers/mcp-server#using-the-mcp-server).
...
If the designer makes changes in Subframe, you'll need to re-export either the component or page code.

**Component updates**

Re-run the sync command to update the component code:
...
Often times, developers will need to modify the component behavior.
