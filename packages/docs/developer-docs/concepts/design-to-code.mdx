...
- **Components** are your design system (e.g. buttons, inputs, cards). They are [synced via CLI](/developer-docs/concepts/syncing-components) to a folder in your codebase (default: `./ui/components`). Each component syncs as a directory — a source file that Subframe generates and you don't modify, plus a wrapper `index.tsx` that you can extend (see [Component directories](/developer-docs/upgrading/component-directories)).
- **Pages** are screens built from components. They are [exported](/developer-docs/concepts/exporting-pages) as copyable React code or using the MCP server and are meant to be modified after export.

In a nutshell, **components are synced, pages are exported**. This is because pages are typically modified with business logic like API calls after export. If your component needs logic after syncing, see our guide on best practices for [exporting components](/developer-docs/concepts/syncing-components).
...
Each component syncs as a directory: the source file Subframe generates (`Button.tsx`) plus a wrapper `index.tsx` that re-exports it and is the import entrypoint. See [Component directories](/developer-docs/upgrading/component-directories) for why, and how to add your own logic in the wrapper.
...
For more information on syncing components, see the [Syncing components](/developer-docs/concepts/syncing-components) guide.
...
- **MCP server (recommended)** - install Subframe MCP server and ask your AI tool to integrate the page code directly using the [page link](/developer-docs/guides/mcp-server#using-the-mcp-server).
...
The best practice is to add your logic to the component's [wrapper `index.tsx`](/developer-docs/concepts/syncing-components#wrapping-components). You probably don't need to sync code back to Subframe, since the source file that Subframe generates stays untouched and keeps receiving design updates. As a last resort, you can [disable sync](/developer-docs/concepts/syncing-components#disabling-sync) for the source file itself.
