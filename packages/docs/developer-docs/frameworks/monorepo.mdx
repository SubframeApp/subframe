...
<Note>
  ...
  information visit the [manual installation guide](/developer-docs/frameworks/manual) and the framework guide for your
  frontend of choice.
</Note>
...
<Note>
  Components and page layouts sync as directories (e.g. `components/Button/`), so each subpath export points at the
  directory's `index.tsx` wrapper rather than a flat `.tsx` file. Subpath patterns are literal — they don't resolve a
  directory to its `index`, so the `/index.tsx` must be explicit. See [Component
  directories](/developer-docs/upgrading/component-directories).
</Note>
