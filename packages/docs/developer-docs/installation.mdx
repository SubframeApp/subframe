  ...
  <Tab title="Existing project">
    ...
    <AccordionGroup>
      ...
      <Accordion title="Importing existing components">
        If you have an existing component library, [import its theme first](/learn/theme/importing-tokens), then use the [Subframe MCP server](/agent/mcp-server) or the `/subframe:design` agent skill to recreate components and snippets one at a time.
      </Accordion>
    </AccordionGroup>
...
See the [MCP server guide](/agent/mcp-server) for available tools and Agent skills.

## FAQ

<AccordionGroup>
  <Accordion title="What if I don't use React, TypeScript or Tailwind CSS?">
    You can use our [MCP server](/agent/mcp-server) to access your designs and documentation in your AI coding assistant and convert them to your preferred language and framework.
