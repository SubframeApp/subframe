...
Customize components using **props** and **slots**.

- **Props** — Simple values like text, image URLs, numbers, or booleans
- **Slots** — React nodes for composition, enabling event handlers and custom components
...
Compare two implementations of a `TrackCard` component from the [Adding business logic](/developing/adding-business-logic#example-2-interactive-track-cards) guide.
...
With **props only**, you pass simple values. Event handlers and framework-specific components aren't accessible.

With **slots**, you pass custom event handlers, framework-specific components, and access subcomponent props.
...
Pass any React element, including complex components with state and logic. Slots support React composition patterns.
