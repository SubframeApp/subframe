...
The left sidebar contains the main theme actions — **Ask AI**, **Import**, **Export** — and lists each section (Colors, Typography, Borders, Corners, Shadows) with token counts. Click any section to scroll to it. The **⋯** menu next to **Theme** holds **Version history** and **Reset theme**.

A header above the Colors section shows the editable theme name and an **Add dark mode** / **Add light mode** button. When dark mode is enabled, a **⋯** menu appears with **Remove dark mode** / **Remove light mode**. See [Dark mode](/guides/dark-mode) for details.
