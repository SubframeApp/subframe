...
<img src="/images/theme/theme-import.webp" alt="Import review screen showing token preview with status indicators for new and updated tokens, with supported source format logos for Tailwind CSS, Figma, Design Tokens, and shadcn/ui" />
...
<Tabs>
  <Tab title="Tailwind CSS (v4)">
    ...
    Include a `.dark` block to import dark mode values alongside their light counterparts. Subframe enables [dark mode](/guides/dark-mode) on the theme and pairs each `.dark` value with its matching light token:
    ...
    Files that only contain a `.dark` block are also supported — Subframe creates standalone dark tokens you can later pair with light values.
  </Tab>
...
### Limitations:
- Figma JSON exports currently support a single mode per import. To bring in both light and dark values from Figma, import light first, then re-import dark — or use the [Tailwind CSS (v4)](#supported-formats) format with `.dark` blocks instead.
