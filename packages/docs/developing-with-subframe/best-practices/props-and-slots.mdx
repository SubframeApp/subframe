...
description: "Learn when and when not to add slots to your components"
---
...
<Tip>
  **Protip:** We can use Subcomponents to expose props and slots to the consumer of a component while giving a hint
  which components are supposed to be used within that slot, as we've done with the `
  <TrackCardWithSlots.FavoriteButton />` subcomponent in the example above.
</Tip>
