...
Since Subframe components are just React components, you can use them like any other React component. You can pass props to them or pass down your component using slots, just like any other React component.
...
Subframe components are built on top of Radix UI, which means that they are accessible by default. They also expose all event handlers that the underlying HTML elements would expose. This means that you can use the `onClick`, `onChange`, and other event handlers to add business logic to your components.
...
For our example app, we'll need to implement some form of login. Magic link logins seem to be a popular choice these days, so let's build a simple magic link login form.
...
In this case, adding business logic to your app is as simple as providing the necessary props to your Subframe components.
...
First, we'll create our component with business logic. Remember that we don't want to change the Subframe component's code directly, so we'll create a new component that wraps the Subframe component and passes our content and events down to it.

Then, we'll create a page component that keeps track of the favorite state and handles the click event.
