<Tabs>
  <Tab title="Claude Code" icon="/images/logos/claude.png">
    ...
    To update to the latest version of the Subframe plugin:

    ```bash
    claude plugin install subframe@subframe
    ```

    To enable auto-updates, add the Subframe plugin to your auto-update list:

    ```bash
    claude settings set autoUpdateMcpServers '["subframe@subframe"]'
    ```

    ### Project-level configuration

    To enable the Subframe plugin for all contributors on a project, add the following to `.claude/settings.json` in your repository:
    ...
    This automatically enables the plugin when contributors open the project with Claude Code.
  </Tab>
