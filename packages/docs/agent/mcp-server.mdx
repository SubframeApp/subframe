---
title: MCP server
description: Connect an external AI agent to Subframe and teach it how to design and implement with your project.
---
...
<Note>Viewers get a read-only MCP server. Read, screenshot, and search tools work, but tools that design or edit require an editor seat.</Note>
...
| Tool                 | Description                                                               |
| -------------------- | ------------------------------------------------------------------------- |
| `list_pages`         | List all pages and the canvases they belong to                            |
| `get_page_info`      | Get a page's generated React and Tailwind code                            |
| `design_page`        | Generate new page designs from a description                              |
| `edit_page`          | Edit a page element's content, structure, and styles                      |
| `update_node_styles` | Replace elements' Tailwind classes while preserving content and structure |
...
| `edit_snippet`       | Edit a snippet element's content, structure, and styles                   |
| `update_node_styles` | Replace elements' Tailwind classes while preserving content and structure |
| `delete_snippet`     | Delete a snippet and remove it from any design documents                  |

### Canvases

A [canvas](/design/canvases) is a collection of related pages (e.g. "Onboarding", "Checkout").

| Tool              | Description                                         |
| ----------------- | --------------------------------------------------- |
| `list_canvases`   | List all canvases                                   |
| `get_canvas_info` | Get a canvas and its pages with their grid position |
| `delete_canvas`   | Delete a canvas, optionally with the pages on it    |

### Organizing

| Tool        | Description                                                                  |
| ----------- | ---------------------------------------------------------------------------- |
| `rename`    | Rename a page, component, snippet, or canvas                                 |
| `move`      | Move pages to another grid cell or canvas; a batch of moves applies together |
| `duplicate` | Make an exact copy of a page, component, snippet, or canvas                  |

A duplicated canvas includes copies of its pages but not its prototype.
...
| `list_icons`          | List uploaded icons and the names of built-in icons               |
| `get_icon_info`       | Get an uploaded icon's markup                                     |
| `list_fonts`          | List Google and custom fonts, with each custom face's URL         |
| `create_asset_upload` | Get an upload link for icon SVGs and font files from your machine |
| `add_icons`           | Add uploaded SVGs to the icon library                             |
| `add_fonts`           | Add uploaded font files as custom font families                   |
| `delete_icons`        | Delete uploaded icons                                             |
| `delete_fonts`        | Delete custom fonts, switching every use to a replacement font    |

The agent uploads files with `curl`, so importing icons and fonts needs an agent that can run commands, like Claude Code, Codex, Cursor, or the Subframe desktop app — the upload link accepts files for 10 minutes, and uploaded files can be added with `add_icons` / `add_fonts` for about a day. A same-name upload replaces the earlier one everywhere it's used; a name that collides with a built-in icon's name or import name (e.g. `FeatherCheck`), or a Google font, is rejected instead.
...
Use `list_components`, `list_pages`, `list_snippets`, or `list_canvases` to see what's available.
