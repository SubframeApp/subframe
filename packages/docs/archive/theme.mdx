---
<<<<<<<< HEAD:packages/docs/tailwind-config.mdx
title: Tailwind configuration
========
title: Theming
>>>>>>>> 46db3f4 (rebase):packages/docs/archive/theme.mdx
---
...
The CLI will add an import to the Subframe `tailwind.config.js` file in your global `tailwind.config.js` file at the root of your app. Please refer to the [Tailwind CSS v3 docs](https://v3.tailwindcss.com/docs/presets) for more information on importing the `tailwind.config.js` file.
...
After [installing Subframe](/installation) your theme will be automatically updated in your codebase. The CLI will create a `theme.css` with your theme tokens.

The CLI will add an import to the `theme.css` file in the global css file that is imported into the root of your app - typically `index.css`, `styles.css`, or `globals.css`. Please refer to the [Tailwind CSS v4 docs](https://tailwindcss.com/docs/theme#sharing-across-projects) for more information on importing the `theme.css` file.
...
Subframe does not currently have a built-in way to theme a project for both light and dark mode, but you can implement dark mode in your codebase using CSS variables. See the [Dark mode](/theme/dark-mode) guide to learn how to set up dark mode in your project.
