...
We've finished [copying the page code](/articles/developing/copy-paste-pages) from Subframe, so now let's add some basic functionality.
...
When adding business logic to Subframe-designed components, we recommend not editing the generated code inside the `src/ui/` directory directly, but instead [wrapping the component](/articles/developing/disabling-sync#wrapping-%E2%80%9Cdumb%E2%80%9D-components-recommended).
...
You may have noticed that we're using slots for our image and favorite button. We'll cover the reasons for this in [Props and Slots](/articles/developing/props-and-slots).
