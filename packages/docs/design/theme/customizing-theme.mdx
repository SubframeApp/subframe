...
A toolbar at the top of the theme page holds the main actions, **Import** and **Export**, plus a **⋯** menu with **Version history**, **Reset theme**, and **Add dark mode** / **Add light mode**. When dark mode is enabled, the menu shows **Remove dark mode** / **Remove light mode** instead. See [Dark mode](/develop/guides/dark-mode) for details.

All changes save automatically and apply immediately across your project.

See also: [Importing tokens](/design/theme/importing-tokens), [Exporting theme](/design/theme/exporting-theme), [Adding custom fonts](/design/theme/adding-custom-fonts)
...
## Edit theme with the agent

{/* TODO: Update image — theme edits now happen in the docked agent chat */}

<img
  src="/images/theme/theme-ask-ai.webp"
  alt="The theme page with an agent prompt open, showing 'update the theme to be brutalist with bold blue accents' with the theme sidebar and color tokens visible"
/>

Use [agent chat](/design/ask-ai/prompt-to-design) to update your theme from a prompt:

1. Press <kbd>Cmd</kbd> + <kbd>/</kbd> to open agent chat
...
3. Send the prompt. The agent updates colors, typography, corners, and shadows

The agent can also rename or delete tokens when the changes call for it. All component references update automatically.

<Note>Theme changes affect your entire project. To undo them, restore an earlier theme from [version history](#version-history).</Note>
...
<img
  src="/images/theme/theme-folders.webp"
  alt="Colors section showing color tokens organized into folders like Text, Background, and Border, with a context menu for moving tokens between folders"
/>
...
To quickly add tokens from an existing design system, see [Importing tokens](/design/theme/importing-tokens).
...
For detailed upload steps and codebase installation, see [Adding custom fonts](/design/theme/adding-custom-fonts).
