...
<Tabs>
  <Tab title="Tailwind CSS (v4)">
    ...
    Subframe detects Tailwind v4 syntax automatically from `.css`, `.scss`, or `.less` files. Tokens declared inside a `.dark { }` block are imported as dark-mode values for their matching light tokens. Importing dark-mode tokens enables dark mode on your current theme automatically — see [Dark mode](/develop/guides/dark-mode). Files with only a `.dark { }` block are also supported.
