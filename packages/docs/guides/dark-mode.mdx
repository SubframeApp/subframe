...
When [dark mode](/learn/theme/dark-mode) is enabled in your theme, the CLI generates CSS variables that switch between light and dark values. Activate dark mode by adding the `dark` class to the `<html>` element.

## Sync your theme

<CliSyncAllCommand />

The CLI generates `:root` variables for light mode and `.dark` overrides for dark mode. See [Exporting theme](/learn/theme/exporting-theme) for details on the generated output.
