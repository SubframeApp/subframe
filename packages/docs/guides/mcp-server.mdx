...
| `edit_theme`         | Edits the visual theme of a project (colors, fonts, corners, shadows) from a natural-language prompt. Best for targeted tweaks or high-level shifts. Enables dark mode automatically when the prompt produces dark variants. Optionally pass a page to preview the change before applying. | `description`<br/>`id`, `name`, or `url` (optional, page to preview)<br/>`projectId` | `editUrl`                                    |
