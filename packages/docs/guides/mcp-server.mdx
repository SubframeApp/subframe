...
The Subframe MCP server gives AI coding assistants like Claude Code, Cursor, and Codex direct access to your Subframe projects. AI can read, design, and delete pages, components, and snippets as well as write design documents and edit the theme.
...
| --- | --- | --- | --- |
| `list_pages` | List all pages | `projectId` | Array of `id`, `name`, `url` |
...
| `edit_theme` | Edits the visual theme of a project (colors, fonts, corners, shadows) from a natural-language prompt. Affects the whole project. Best for targeted tweaks or high-level shifts. When a page identifier is provided, the user can preview before applying; otherwise, the theme is applied immediately | `description`, `id`/`name`/`url?`, `projectId` | `editUrl` |
