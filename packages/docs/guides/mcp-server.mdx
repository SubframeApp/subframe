...
Most tools take a `projectId` — omit it and the first project you have access to is used. You can find a project's ID in its editor URL (`app.subframe.com/PROJECT_ID/...`), or in `.subframe/sync.json` once you've run the CLI.
...
Images must be uploaded to Subframe; external URLs are rejected, and up to 5 images are used per call.
