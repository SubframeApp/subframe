...
import { cwd as defaultCwd } from "./common"
...
export const syncCommand = new Command()
  ...
  .action((args, options) => syncFunction(args, options, undefined))

const options = syncCommand.opts()

export async function syncFunction(components: string[], opts: typeof options, cwd: string | undefined = defaultCwd) {
  ...
  try {
    ...
    await installDependencies(cwd, { install: undefined })
